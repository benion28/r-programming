...
tree_data <- data.frame(
  ...
  Wood_Density = numeric(num_trees),
...
# Dictionary mapping species to wood density (in kg/m^3)
wood_density_mapping <- list(
  "Quercus robur" = 700,
  "Pinus sylvestris" = 450,
  "Acer saccharum" = 710,
  "Fagus sylvatica" = 670,
  "Picea abies" = 430,
  "Fraxinus excelsior" = 670,
  "Prunus avium" = 720,
  "Betula pendula" = 670,
  "Cedrus libani" = 560,
  "Cupressus sempervirens" = 470,
  "Abies alba" = 420,
  "Tsuga canadensis" = 450,
  "Larix decidua" = 590,
  "Robinia pseudoacacia" = 690,
  "Magnolia grandiflora" = 600,
  "Populus tremula" = 420,
  "Salix alba" = 470,
  "Taxus baccata" = 640,
  "Juglans regia" = 610,
  "Sequoia sempervirens" = 480
  # Add more species with their corresponding wood densities
)

unique_species <- names(species_mapping)
num_unique_species <- length(unique_species)

# Generating correlated DBH and Tree Height
# correlation_coefficient <- 0.8
# cov_matrix <- matrix(c(1, correlation_coefficient, correlation_coefficient, 1), ncol = 2)
# correlated_values <- MASS::mvrnorm(n = num_trees, mu = c(0, 0), Sigma = cov_matrix)

# Generating correlated DBH, Tree Height and Crown Diameter
correlation_coefficient <- 0.8
correlation_matrix <- matrix(c(1.0, correlation_coefficient, 0.7,
          correlation_coefficient, 1.0, 0.8, 0.7, 0.8, 1.0), ncol = 3)
...
for (index in 1:num_trees) {
  ...
  tree_data$DBH[index] <- abs(correlated_values[index, 1] * 20) + 40  # in centimeters
  tree_data$Crown_Diameter[index] <- abs(correlated_values[index, 3] * 5) + 3  # in meters
  ...
  # Assigning wood density based on the selected species
  tree_species <- tree_data$Scientific_Name[index]
  tree_data$Wood_Density[index] <- wood_density_mapping[[tree_species]]/1000
