...
# Calculate RSCORE
RSQUARED = cor(testing$H, predictions) # Correlation Coefficient
...
ggplot() +
  geom_point(aes(x = testing$D, y = testing$H), colour = "red") +
  geom_line(aes(x = testing$D, y = y_predict), colour = "green")+
  ggtitle("Tree Height-Diameter Relationship (Linear Regression Line)") +
  xlab("Diameter") +
  ylab("Height")
...
ggplot() +
  geom_line(aes(x = testing$D, y = y_predict), colour = "green")+
  ggtitle("Tree Height-Diameter Relationship (Linear Regression Line Only)") +
  xlab("Diameter") +
  ylab("Height")
...
ln_predict <- predict(linear_model, predictor_variable)
svr_predict <- predict(model, predictor_variable)
