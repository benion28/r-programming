...
# ----------------------------- Power Model----------------------------- #
# Power Model Fitting
model_power <- nls(formula = HEIGHT ~ (1.3 + a * DIAMETER^b), data = training, start=list(a=start_values_power["a"], b=start_values_power["b"]))
...
model_chapman <- nls(formula = HEIGHT ~ (1.3 + a * (1 - exp(-b * DIAMETER))^c), data = training, start=list(a=start_values_chapman["a"], b=start_values_chapman["b"], c=start_values_chapman["c"]))
...
WB <- deriv( ~1.3 + a * (1 - exp(-b * x^c)) , c("a","b","c"), function(x,a,b,c){})
WB.nls <- nls(HEIGHT ~ WB(DIAMETER, a, b, c), data = training, 
              start = c(a = 10, b = 0.3, c = 0.3)) 
...
predictions_weibull = predict(WB.nls, testing)
...
# Weibull
predict_logistics <- predict(WB.nls, predictor_variable)
...
residuals(model_power)
res_power<- as.data.frame(residuals(model_power))
View(res_power)
summary(model_chapman)
...
MAPE <- c(
  ...
  round(MAPE_ann, 4),
  round(MAPE_power, 4),
  round(MAPE_chapman, 4),
  round(MAPE_logistics, 4),
  round(MAPE_weibull, 4)
  )
...
Accuracy <- c(
  round(accuracy_svr, 1), 
  round(accuracy_ann, 1), 
  round(accuracy_power, 1), 
  round(accuracy_chapman, 1), 
  round(accuracy_logistics, 1), 
  round(accuracy_weibull, 1)
  )
...
ggplot() +
  ...
  theme(legend.position = c(0.90, -0.07), legend.justification = c("right", "bottom")) +
...
# Scattered Plot (Model Performance Line/Curve) -- Power
ggplot() +
  geom_point(aes(x = testing$DIAMETER, y = testing$HEIGHT), colour = "red") +
  geom_line(aes(x = testing$DIAMETER, y = predictions_power), colour = "black") +
  labs(x = "Diameter", y = "Height", title = "Tree Height - Diameter Model Line (Power)") 
