...
setwd("C://Benion//Benion Programmings//R//Analysis Result//20 CD-D")


# Import data set
dataset <-  read.csv("full-data-8.csv", head=TRUE, sep=",")
head(dataset, n = 10)
summary(dataset)
describe(dataset)
...
training <- read.csv("training-data-8.csv", head=TRUE, sep=",")
summary(training)
training_description <- describe(training)

# Testing or Validation Data
testing <- read.csv("testing-data-8.csv", head=TRUE, sep=",")
summary(testing)
