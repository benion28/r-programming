...
setwd("C://Benion//Benion Programmings//R//Analysis Result//26 CD-D")
...
training <- read.csv("training-data-11.csv", head=TRUE, sep=",")
summary(training)
training_description <- describe(training)

# Testing or Validation Data
testing <- read.csv("testing-data-11.csv", head=TRUE, sep=",")
summary(testing)
...
# Scattered Plot (Fitted VS Residuals Values) -- Model 3
ggplot() +
  geom_point(aes(x = fitted(model_3), y = residuals(model_3), colour = "Points")) +
  geom_abline(aes(slope=0, intercept=0, colour = "Model 3")) +
  labs(x = "Fitted Values", y = "Standardized Residuals", title = "Tree Crown Diameter - Diameter -- Fitted vs Residuals (Model 3)", color = "Legend") +
  theme(legend.position = c(0.99, 0.05), legend.justification = c("right", "bottom")) +
  scale_color_manual(values = c("Points"="#a32e2e", "Model 3"="#4954ed")) 

plot(model_3, col = "black",
     main = "Tree H-D Fitted VS Residuals Values -- Model 3",
     xlab = "Fitted Values",
     ylab = "Standardized Residuals") 
...
# Scattered Plot (Standardized Normal Quantile vs Standardized Residuals) -- Model 3
ggplot(residuals_table, aes(sample = Model.3)) +
  geom_qq(color = "#a32e2e") +
  geom_qq_line(color = "#4954ed") +
  labs(x = "Quantiles of Standardized Normal", y = "Standardized Residuals", title = "Tree CD-D Standardized Normal Quantile vs Residuals (Model 3)")

qqnorm(residuals(model_3, type = "pearson"), 
       main = "Tree H-D Standardized Normal Quantile vs Standardized Residuals -- Model 3", 
       xlab = "Standardized Normal Quantile", ylab = "Standardized Residuals")
qqline(residuals(model_3,type = "pearson"))
