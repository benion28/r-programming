...
setwd("C://Benion//Benion Programmings//R//Analysis Result//7 HD-D")
...
training <- read.csv("training-data-7.csv", head=TRUE, sep=",")
summary(training)
training_description <- describe(training)

# Testing or Validation Data
testing <- read.csv("testing-data-7.csv", head=TRUE, sep=",")
summary(testing)
...
parameters_nls <- data.frame(
  "a" = c(round(summary_model_power[["parameters"]][1], 3), round(summary_model_curtis[["parameters"]][1], 3), round(summary_model_logistics[["parameters"]][1], 3), round(summary_model_naslund[["parameters"]][1], 3)),
  "b" = c(round(summary_model_power[["parameters"]][2], 3), round(summary_model_curtis[["parameters"]][2], 3), round(summary_model_logistics[["parameters"]][2], 3), round(summary_model_naslund[["parameters"]][2], 3)),
  "c" = c("-", "-", round(summary_model_logistics[["parameters"]][3], 3), "-")
)
...
rownames(summary_table_training) <- c("Power", "Curtis", "Logistics", "Naslund")
rownames(summary_table_testing) <- c("Power", "Curtis", "Logistics", "Naslund")
...
ggplot() +
  ...
  theme(legend.position = c(0.98, 0.05), legend.justification = c("right", "bottom")) +
...
ggplot() +
  ...
  theme(legend.position = c(0.80, 0.05), legend.justification = c("right", "bottom")) +
  scale_color_manual(values = c("Points"="#a32e2e", "Naslund"="#4954ed")) 

plot(model_naslund, col = "black",
     main = "Tree H-D Fitted VS Residuals Values -- Naslund",
     xlab = "Fitted Values",
     ylab = "Standardized Residuals") 
...
qqnorm(residuals(model_naslund, type = "pearson"), 
       main = "Tree H-D Standardized Normal Quantile vs Standardized Residuals -- Naslund", 
       xlab = "Standardized Normal Quantile", ylab = "Standardized Residuals")
qqline(residuals(model_naslund,type = "pearson"))
